...
| Use goals, or a goals/xG blend, as the in-season form signal | **Reject.** Pooled over 2022/23-2025/26, every blend weight from 25% to 100% has a clean-sheet Brier interval crossing zero, and full goals is worse on whole-model xP in 2024/25. But §3.3 of `calculations.md` overstates the loss: pooled Brier is -0.00009, a wash rather than a defeat. `results/goals-vs-xg.md`. |
| Steepen the opponent-attack multiplier in the clean-sheet read | **Reject, resolved.** An exponent of 1.2 costs +0.00152 [+0.00076, +0.00222] pooled and 1.4 costs +0.00370 [+0.00219, +0.00504]. `results/goals-vs-xg.md`. |
| Change the current-season weight on goalkeeper saves from `n/(n+6)` | **No.** Pooled over 3,002 goalkeeper appearances the RMSE curve is flat between 6 and 15 and both ends are resolved against: `n/(n+2)` costs +0.00961 [+0.00281, +0.01599] and previous-season-only +0.01920 [+0.00542, +0.03552]. Weighting the anchor by the matches it was built from also loses (+0.00412, unresolved). The gameweek 2-5 window resolves nothing. Measurable only since the ingest began carrying per-match saves. `results/saves-weight.md`. |
| Take the clean-sheet level from Elo alone | **Rejected; changed.** Freezing the level to Elo left a uniformly declining side with an unchanged rating, and rated a side whose attack fell faster than its defence as defending better. Handing the level to the in-season fit on `n/(n+12)`, stretched to Elo's spread, is -0.00123 Brier pooled [-0.00219, -0.00047] with AUC 0.638 to 0.648, negative in all four seasons and resolved in both the gameweek 2-5 and 6-38 windows on the converged Elo seasons. Shipped in `lib/projections/cleanSheetStrength.ts`. `results/cs-level-source.md`. |
| Use a team's own clean-sheet rate as the defensive signal | **Reject.** Applied to the goals-against mean it is worse at every weight above 0.25 (pooled +0.00164 at 0.5, +0.00566 at full) and AUC falls monotonically 0.648 to 0.625, so it scrambles the fixture ordering rather than mis-levelling it. Damped through the skew transform it is flat pooled and flips sign by season. `results/cs-rate-signal.md`. |
| Compress the rated clean-sheet read toward the long-run rate | **Open.** The rated path predicts a 0.274 clean-sheet rate against a measured 0.241-0.252, and shrinking toward 0.25 at 0.85 is -0.00069 [-0.00120, -0.00004] pooled. Flattening the opponent exponent buys the same number and does not stack with it, so the two are one effect. Unresolved in three seasons of four, and worse in the one season that realized 0.285. `results/goals-vs-xg.md`. |
...
| Reparameterise goalkeeper saves as team lambda times keeper save percentage | **Reject.** Pooled over 3,002 goalkeeper appearances, saves RMSE is worse at every kappa with the interval excluding zero (k=100 +0.05092 [+0.03371, +0.07148]; k=Inf +0.03735 [+0.01717, +0.06056]), and the joint shots total is worse too — the keeper's own rate carries volume signal lambda does not replace. The implied save-percentage spread narrows as designed without buying accuracy. Goalkeeper xP is unresolved throughout. Measurable only since the ingest began carrying per-match goals conceded. `results/coherent-saves.md`. |
...
npx tsx scripts/backtest/coherent-saves.ts    # team-lambda x keeper-save-percentage vs the shipped saves rate
```
