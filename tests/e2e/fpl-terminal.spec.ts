...
test.describe("FPL Terminal acceptance", () => {
  ...
  async function lockPlayer(page: Page, name: string) {
    ...
    await clickButton(page, new RegExp(`lock.*${name}|${name}.*lock`, "i"));
  }
  ...
  test("pastes an existing squad, inspects replacements, simulates, and applies a move", async ({ page }) => {
    ...
    await expect(page.getByText(/squad analysis|total cost|projected|weakest links/i).first()).toBeVisible();
    await clickButton(page, /replacement/i);
    await expect(page.getByText(/replacement|alternative|upgrade/i).first()).toBeVisible();

    await clickButton(page, /simulate/i);
  ...
  test("keeps the player universe dense and gives each desktop panel an accessible minimize control", async ({ page }) => {
    ...
    const headers = (await universe.getByRole("columnheader").allTextContents()).join(" ").replace(/\s+/g, " ");
    for (const label of [/own(?:ership)?%?/i, /(?:gw\s*xp|xp\s*gw)/i, /(?:3gw|3\s*gw|xp\s*3)/i, /(?:5gw|5\s*gw|xp\s*5)/i, /xp\s*\/\s*£/i, /(?:exp|expected)\s*min/i, /risk/i, /fixtures/i]) {
      expect(headers, `player universe should expose ${label}`).toMatch(label);
    }

    for (const name of [/player universe/i, /squad builder/i, /squad analysis/i, /ai analyst/i]) {
